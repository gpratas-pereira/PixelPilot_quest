-- Rewards System Supabase Migration
...
-- Create pilot_achievements table (progress tracking)
...
CREATE INDEX IF NOT EXISTS idx_pilot_achievements_unlocked ON pilot_achievements(unlocked_at) WHERE unlocked_at IS NOT NULL;
...
('Race Entry Ticket', 'Single race entry with basic amenities', 'ticket', 300, 3.00, -1, true)
...
('Points Collector', 'Earn 10,000 lifetime points', 2000, 'lifetime_points', '{"target": 10000}', 3)
ON CONFLICT DO NOTHING;

-- Insert sample point rules
...
('achievement_unlock', 'Unlock an achievement', 0, true), -- Points awarded by achievement itself
('marketplace_purchase', 'Purchase item from marketplace', 0, true) -- No base points for purchases
ON CONFLICT DO NOTHING;
